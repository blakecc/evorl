...
# Add model directory argument
parser.add_argument('--model-dir', default="/test_models/testmod.p", help='Model directory path')
...
def create_commands(session, num_workers, remotes, env_id, logdir, modeldir, shell='bash', mode='tmux', visualise=False):
    # for launching the TF workers and for launching tensorboard
    base_cmd = [
        'CUDA_VISIBLE_DEVICES=',
        sys.executable, 'worker.py', #sys.executable is just the python program path
        '--log-dir', logdir,
        '--env-id', env_id,
    ...
    for i in range(num_workers):
        cmds_map += [new_cmd(session,
            "w-%d" % i, base_cmd + ["--job-name", "worker", "--task", str(i), "--remotes", remotes[i], "--model-dir", modeldir], mode, logdir, shell)]
...
def run():
    ...
    # model_path = "/Users/blakecuningham/Dropbox/MScDataScience/Thesis/EDEN/test_models/testmod.p"
    cmds, notes = create_commands("a3c", args.num_workers, args.remotes, args.env_id, args.log_dir, args.model_dir, mode=args.mode, visualise=args.visualise)
    if args.dry_run:
